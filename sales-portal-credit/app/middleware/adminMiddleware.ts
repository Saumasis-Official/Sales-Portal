            ...
            if (token) {
                ...
                if (!payload || payload.client_id !== adminConfig.cognitoClientId) {
                    ...
                    if (adminDetails && adminDetails.rows && adminDetails.rows.length) {
                        ...
                        if(_.intersection(adminRole, [...pegasus.ADMIN, ...pegasus.MT_ECOM, ...pegasus.CREDIT_LIMIT]).length){
                            logger.info('Admin Middleware Super admin/Credit limit approvers/KAMS');
                            if (distributorId) {
                                let validated = false;
                        ...
                        } else if (_.intersection(adminRole, pegasus.SALES).length) {
