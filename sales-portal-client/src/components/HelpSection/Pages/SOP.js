...
function SopTab(props) {
  ...
  const handleClick = (i, file) => {
    const url = getPreAssignedUrl(file).then(url => {
    ...
    <div className="SecondTab">
      <Collapse onChange={onChange}>
        {get_help_section_data?.data?.response?.rows?.map((data, i) => {
          return (
            <Panel header={data?.title} key={i} style={{ fontWeight: '500', fontSize: '15px' }}  >
              <div>
                <p>{data?.description}</p>
                <div>
                  {data?.file_path?.length > 0 ? <h4>Attached Help Documents</h4> : ''}
                </div >
                          ...
                          {file.split('.')[1] == 'mp4' ? <Tooltip placement="top" title={'Click here to play video'}>
                            <div className="video-card" onClick={() => handleClick(i, file)}>
                              <Card
                                ...
                                className='vid-card'
                              >
                                {<img style={{ width: 120 }} className='images' alt="img not available" src="/assets/images/video.svg" />}
                ...
                {
                  (data && data.contact_name && data.contact_number && data.email) ? <div className='contact-container' style={{ margin: '15px 10px' }}>
                    <div style={{ fontSize: '1.2rem' }}>Contact Person Details</div>
                    <di style={{ fontWeight: 'normal' }} >{(data.contact_name == 'undefined') ? '' : data.contact_name}</di>
                    <div style={{ fontWeight: 'normal' }}>{data.contact_number == 'undefined' ? '' : data.contact_number}</div>
                    <div style={{ fontWeight: 'normal' }}>{data.email == 'undefined' ? '' : data.email}</div>
                  </div> : ''
                }
              </div>
            </Panel>
...
const connectSOP = connect(
  mapStateToProps,
  mapDispatchToProps,
)(SopTab);

export default connectSOP;