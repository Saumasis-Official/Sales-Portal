...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'roles_type') THEN
        IF EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'APPROVER(PRIMARY)' AND enumtypid = 'roles_type'::regtype) THEN
            ALTER TYPE roles_type RENAME VALUE 'APPROVER(PRIMARY)' TO 'CL_PRIMARY_APPROVER';
        END IF;
 
        IF EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'APPROVER(SECONDARY)' AND enumtypid = 'roles_type'::regtype) THEN
            ALTER TYPE roles_type RENAME VALUE 'APPROVER(SECONDARY)' TO 'CL_SECONDARY_APPROVER';
        END IF;
    END IF;
END $$;
