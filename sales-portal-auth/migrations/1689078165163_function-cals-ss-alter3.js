...
exports.up = pgm => {
    pgm.sql(`
            create or replace function calc_ss(p_db text, p_area text) 
            returns jsonb
            as $$
                declare  
                ss int;
                ...
                month_str text := '';
                begin 
                    SELECT max(snc.safety_stock)::int
                    INTO  ss
                    FROM stock_norm_configuration AS snc 
                    WHERE snc.area_code = p_area;
                    ...
                    if pos is  null then return jsonb_build_object('psku',psku_arr,'ss',ss_arr);
                    end if;
                    ...
                    if position('f_d.' in month_str)= 0 then
					EXECUTE format('SELECT array_agg(fd.psku), array_agg((%s)/mc.base_to_case) 
                                    ...
                                    WHERE fd.applicable_month = CONCAT(to_char(current_date, ''YYYY''),to_char(current_date, ''MM'')) AND 
                                        fd.distributor_code = ''%s''',month_str,p_db,p_db)
                    ...
                    if psku_arr is null then return jsonb_build_object('psku',array[]::text[],'ss',array[]::text[]);
                    end if;
