...
const ConnectToS3Bucket = {
    ...
    async uploadForecastFile(file, name: string, type: string) {
        try {
            const path = ConnectToS3Bucket.getForecastS3Path(type) || '';
    ...
    async listFilesInS3(bucketName: string, prefix: string) {
        try {
            const params = {
                Bucket: bucketName,
                Prefix: prefix,
            ...
            logger.info(`Files listed successfully from S3: ${prefix}`);
    ...
    async deleteFilesInS3(bucketName: string, filePaths: string[]) {
        try {
            const deleteParams = {
                Bucket: bucketName,
                Delete: {
            ...
            const data = await awsS3Client.deleteObjects(deleteParams).promise();
            logger.info(`Files deleted successfully from S3`, data);
            return data;
        } catch (error) {
