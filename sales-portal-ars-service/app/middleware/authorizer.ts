...
const authorizer = {
    ...
    getMoqMappingData(req: Request, res: Response, next) {
        const accessTo = [...pegasus.ADMIN, ...pegasus.LOGISTICS];
        hasPermission(userRole(req), accessTo) ? next() : unauthorizedResponse(res);
    },

    updateMoq(req: Request, res: Response, next) {
        const accessTo = [roles.LOGISTIC_OFFICER, roles.SUPER_ADMIN, roles.PORTAL_OPERATIONS];
        hasPermission(userRole(req), accessTo) ? next() : unauthorizedResponse(res);
    },
