            ...
            if (token) {
                ...
                if (!payload || payload.client_id !== adminConfig.cognitoClientId) {
                    ...
                    if (adminDetails && adminDetails.rows && adminDetails.rows.length) {
                        ...
                        if ((_.intersection(adminRole, ["SUPER_ADMIN", "SUPPORT", "LOGISTIC_OFFICER", "ZONAL_OFFICER", "OPERATIONS", "SHOPPER_MARKETING", "PORTAL_OPERATIONS", "FINANCE", "VP", "CALL_CENTRE_OPERATIONS", "FINANCE_CONTROLLER"])).length) {
