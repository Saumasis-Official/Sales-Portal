...
export const LogModel = {
    ...
    async insertSMSLogs(
        ...
        logger.info("inside LogModel -> insertSMSLogs");
        let client: PoolClient | null = null;
        try {
            client = await conn.getWriteClient();
            const sqlStatement = `INSERT INTO audit.sms_logs (type, status, recipients, reference, sms_data, error_logs) VALUES ($1, $2, $3, $4, $5, $6);`;
            const response = await client.query(sqlStatement, [type, status, JSON.stringify(recipients), reference, sms_data, error]);
            return response?.rowCount;
        } catch (error) {
            logger.error("CAUGHT: Error in LogModel -> insertSMSLogs", error); 
            return null;
        } finally {
            client?.release();
        }
