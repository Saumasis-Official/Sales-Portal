    ...
    */
    checkOrderAllowed_WE_PDP(day, today, pdpFrequency, pdpConfig: PDPConfig) {
        ...
        */
        ...
        const orderPlacementEndTimeMinutes = (orderPlacementEndTime.charAt(0) === '-' ? (0 - Number(orderPlacementEndTime.split(':')[1])) : Number(orderPlacementEndTime.split(':')[1])) | 0;
        const days = {
            'SU': 0,
            'MO': 1,
            'TU': 2,
        ...
        let upcomingPDPDay = (((pdpWeekDay - todayWeekDay) < 0) || ((pdpWeekDay - todayWeekDay) === 0 && orderPlacementEndTimeHour < today.getHours())) ? (pdpWeekDay - todayWeekDay) + 7 : (pdpWeekDay - todayWeekDay);
    ...
    checkOrderAllowed_FN_PDP(day, referenceDate, today, pdpFrequency, pdpConfig: PDPConfig) {
        ...
        */
        ...
        while ((upcomingPDPDate.getFullYear() < today.getFullYear()) ||
            (upcomingPDPDate.getFullYear() === today.getFullYear() && Number(upcomingPDPDateWeekNumber) < Number(todayWeekNumber)) ||
            (upcomingPDPDate.getFullYear() === today.getFullYear() && Number(upcomingPDPDateWeekNumber) === Number(todayWeekNumber) && Number(upcomingPDPDate.getDay()) < Number(todayWeekDay)) ||
            (upcomingPDPDate.getFullYear() === today.getFullYear() && Number(upcomingPDPDateWeekNumber) === Number(todayWeekNumber) && Number(upcomingPDPDate.getDay()) >= Number(todayWeekDay) && orderEndDate < today)
        ) {
