...
const authorizer = {
    ...
    adminTeamTSEAndAbove(req: Request, res: Response, next) {
        const accessTo: string[] = [roles.SUPER_ADMIN, roles.PORTAL_OPERATIONS, roles.DIST_ADMIN, roles.ASM, roles.RSM, roles.CLUSTER_MANAGER, roles.TSE];
        hasPermission(userRole(req), accessTo) ? next() : unauthorizedResponse(res);
    },

    getBulkOrderMappingData(req: Request, res: Response, next) {
        const accessTo = [...pegasus.ADMIN, ...pegasus.LOGISTICS];
        hasPermission(userRole(req), accessTo) ? next() : unauthorizedResponse(res);
    },
    bulkOrderupdateMoq(req: Request, res: Response, next) {
        const accessTo = [roles.SUPER_ADMIN, roles.PORTAL_OPERATIONS, roles.LOGISTIC_OFFICER, roles.ZONAL_OFFICER]
        hasPermission(userRole(req), accessTo) ? next() : unauthorizedResponse(res);
    },
    mappedAreaZones(req: Request, res: Response, next) {
        const accessTo = [roles.SUPER_ADMIN, roles.PORTAL_OPERATIONS, roles.SUPPORT, ...pegasus.LOGISTICS]
        hasPermission(userRole(req), accessTo) ? next() : unauthorizedResponse(res);
    },

    updateSDRequest(req: Request, res: Response, next) {
        const accessTo = pegasus.LOGISTICS;
        hasPermission(userRole(req), accessTo) ? next() : unauthorizedResponse(res);
    },
}
