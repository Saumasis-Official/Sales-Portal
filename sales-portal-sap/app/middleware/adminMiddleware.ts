  ...
  async validateToken(req, res, next) {
    try {
      ...
      if (token) {
        ...
        if (
          !payload ||
          payload.client_id !== adminConfig.cognitoClientId
        ) {
          logger.error(
            `Admin Middleware If client id does not match`,
          );

          res
            .status(403)
            .json(responseTemplate.commonAuthUserDataError());
        } else {
          const username = payload.username;
          const email = username.replace(
            adminConfig.cognitoIdpName,
            '',
          );
          ...
          let adminDetails: any =
            await AdminService.adminDetailsStatement(email);
          logger.info(
            'Admin Middleware adminDetails',
          ...
          if (
            ...
            if ((_.intersection(adminRole, ["SUPER_ADMIN", "SUPPORT", "LOGISTIC_OFFICER", "ZONAL_OFFICER", "OPERATIONS", "SHOPPER_MARKETING", "PORTAL_OPERATIONS", "FINANCE", "VP", "CALL_CENTRE_OPERATIONS", "FINANCE_CONTROLLER", "CUSTOMER_SERVICE"])).length) {
              ...
              if (distributorId) {
                ...
                const validateAdminResponse =
                  await AdminService.validateSuperAdminStatement(
                    distributorId,
                  );
                logger.info(
                  'Admin Middleware Super admin/Support/CCO: ',
                  validateAdminResponse && validateAdminResponse.rows,
                );
                if (
                  validateAdminResponse &&
                  validateAdminResponse.rows &&
                  validateAdminResponse.rows.length
                ) {
                  validated = true;
                }
                if (validated) next();
            ...
            } else if ((_.intersection(adminRole, ["KAMS", "MDM", "NKAMS", "SHOPIFY_UK", "SHOPIFY_SUPPORT", "SHOPIFY_OBSERVER", roles.CL_PRIMARY_APPROVER, roles.CL_SECONDARY_APPROVER, roles.RCM, roles.HOF, roles.GT_PRIMARY_APPROVER, roles.GT_SECONDARY_APPROVER])).length) {
              next();
            }
            else {
              res
        ...
        res
          .status(403)
          .json(responseTemplate.tokenRequiredAuthError());
      }
    } catch (error) {
