...
export class ValidateSelfLiftingOrder implements ValidateOrderInterface {
    public order_type: OrderTypes  = OrderTypes.SELF_LIFTING;
    public orderType: string = 'SELF_LIFTING';
    static #instance: ValidateSelfLiftingOrder;

    private constructor() {
    }

    public static getInstance(): ValidateSelfLiftingOrder {
        if(!this.#instance){
            this.#instance = new ValidateSelfLiftingOrder();
        }
    ...
    public validatePayload(order_data: any): { status: boolean; message: string; } {
        ...
        try{
            ...
            logger.error('inside validateSelfLiftingOrder -> validatePayload, Error: ', e);
            response.status = false;
    ...
    public async validate(order_data: any, distributor_id:string, user: any): Promise<ValidateOrderResponse> {
        ...
        try{
            ...
            order_data['distribution_channel'] = '40';
            ...
            if(status === 200 || status === 201){
                ...
                response.status = false;
                response.message = message || ErrorMessage.VALIDATE_SELF_LIFTING;
                response.data = data || null
            }

        }catch(e){
            logger.error('inside validateSelfLiftingOrder, Error: ', e);
            response.status = false;
            response.message = (<Error>e).message || ErrorMessage.VALIDATE_SELF_LIFTING;
            response.data = null;
        }finally{
            return response;
        }
    ...
    public async saveOrder(order_data: any, sap_payload: any, sap_result: any, distributor_id: string, user: any): Promise<{status: boolean, message: string, data: any}> {
        ...
        logger.info('inside validateSelfLiftingOrder -> saveOrder');
        try{
            ...
            const initialOrderItemsMap = order_data['items']
                                            ...
                                            , {});
            const itemset = sap_payload?.Itemset.map((item: SapValidationPayloadItems) => {
            ...
            if (insertResult?.rowCount) {
                logger.info(`inside validateSelfLiftingOrder -> saveOrder, PO: ${order_data.po_number} saved successfully`);
                ...
                logger.info(`inside validateSelfLiftingOrder -> saveOrder, failed to save PO: ${order_data.po_number}`);
            }
    

        }catch(error){
            logger.error('inside validateSelfLiftingOrder -> saveOrder, Error: ', error);
