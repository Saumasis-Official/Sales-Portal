...
export class CreateRegularOrder implements CreateOrderInterface {
    public order_type: OrderTypes = OrderTypes.NORMAL;
    public orderType: string = 'NORMAL';
    static #instance: CreateRegularOrder;

    private constructor() {}

    public static getInstance(): CreateRegularOrder {
        if (!this.#instance) {
            this.#instance = new CreateRegularOrder();
        }
    ...
    public createPayload(order_details: any): CreateOrderSAPPayload {
        try {
            logger.info('inside CreateRegularOrder -> createPayload');
            ...
            logger.error('inside CreateRegularOrder -> validatePayload, Error: ', e);
            throw e;
        } 
    }
    ...
    public async create(po_number: string, user: any): Promise<CreateOrderResponse> {
        try {
            ...
            if(!po_number.startsWith('DBO') && !po_number.startsWith('CCO')){
                throw new Error('Invalid Regular PO number');
            }
            ...
            for (let item of order_data.Itemset) {
                if (!isCCO && isGlobalPDPOn && dbData.enable_pdp && !(item.CMIR === 'X') && item.pdp_day) {
            ...
            if(!createOrderResponse || !(createOrderResponse.status === 200 || createOrderResponse.status === 201) || !createOrderResponse.data){
                logger.info('inside CreateRegularOrder -> create, Error: SAP failure response - ', createOrderResponse);
                return {
                    status: false,
            ...
            logger.info('inside CreateRegularOrder -> create, SAP success response data ',response_data);
            ...
            for(let nav_result of nav_results){
                ...
                if (!poNumber || !soNumber || !soValue) {
                    logger.info('inside CreateRegularOrder -> create, Error: SAP response does not contain PoNumber or SalesOrder or Sales Value');
                    return {
                        status: false,
                ...
                if(!nav_result['Ship_to'] || nav_result['Ship_to'] !== sap_payload.ShipTo){
                    logger.info('inside CreateRegularOrder -> create, Error: SAP response does not contain Ship_to or Ship_to point does not match with payload');
                    return {
                        status: false,
                ...
                if(!nav_result['Sold_to'] || nav_result['Sold_to'] !== sap_payload.SoldTo){
                    logger.info('inside CreateRegularOrder -> create, Error: SAP response does not contain Sold_to or Sold_to does not match with payload');
                    return {
                        status: false,
                ...
                if(!nav_result['Unloading'] || nav_result['Unloading'] !== sap_payload.Unloading){
                    logger.info('inside CreateRegularOrder -> create, Error: SAP response does not contain Unloading or Unloading point does not match with payload');
                    return {
                        status: false,
                ...
                if(!updateOrdersResult || updateOrdersResult.rowCount === 0){
                    logger.info('inside CreateRegularOrder -> create, Error: Failed to update order in database');
                    return {
                        status: false,
                        message: 'Regular order created in SAP but failed to update order in database',
                        data: createOrderResponse.data,
                    };
                }
                logger.info('inside CreateRegularOrder -> create, Order updated in database successfully');
                ...
                logger.info('inside CreateRegularOrder -> create, notificationpreferanceResult: ',selectUserNotificationResult?.rows[0]);
                if (!selectUserNotificationResult?.rows?.length) {
                    logger.info('inside CreateRegularOrder -> create, notification preference does not exists for distributor- '+ distributor_id);
                    continue;
                }
                ...
                if (!adminData || !Object.keys(adminData).length) {
                    logger.info('inside CreateRegularOrder -> create, Error: Admin data not found for distributor- '+ distributor_id);
                    continue;
                }
                //Sending email
                if (isEmailEnabled) {
            ...
            logger.error('inside CreateRegularOrder -> create, Error: ', e);
            throw e;
        }
    }
}
