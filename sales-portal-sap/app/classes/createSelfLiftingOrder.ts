...
export class CreateSelfLiftingOrder implements CreateOrderInterface {
    public order_type: OrderTypes = OrderTypes.SELF_LIFTING;
    public orderType: string = 'SELF_LIFTING';
    static #instance: CreateSelfLiftingOrder;

    private constructor() {}

    public static getInstance(): CreateSelfLiftingOrder {
        if (!this.#instance) {
            this.#instance = new CreateSelfLiftingOrder();
        }
    ...
    public createPayload(order_details: any): CreateOrderSAPPayload {
        try {
            logger.info('inside CreateSelfLiftingOrder -> createPayload');
            ...
            logger.error('inside CreateSelfLiftingOrder -> validatePayload, Error: ', e);
            throw e;
        } 
    }
    ...
    public async create(po_number: string, user: any): Promise<CreateOrderResponse> {
        try {
            if(!po_number.startsWith('SFL')){
                throw new Error('Invalid Self Lifting PO number');
            }
            ...
            if(!createOrderResponse || !(createOrderResponse.status === 200 || createOrderResponse.status === 201) || !createOrderResponse.data){
                logger.info('inside CreateSelfLiftingOrder -> create, Error: SAP failure response - ', createOrderResponse);
                return {
                    status: false,
            ...
            logger.info('inside CreateSelfLiftingOrder -> create, SAP success response data ',response_data);
            ...
            for(let nav_result of nav_results){
                ...
                if (!poNumber || !soNumber || !soValue) {
                    logger.info('inside CreateSelfLiftingOrder -> create, Error: SAP response does not contain PoNumber or SalesOrder or Sales Value');
                    return {
                        status: false,
                ...
                if(!nav_result['Ship_to'] || nav_result['Ship_to'] !== sap_payload.ShipTo){
                    logger.info('inside CreateSelfLiftingOrder -> create, Error: SAP response does not contain Ship_to or Ship_to point does not match with payload');
                    return {
                        status: false,
                ...
                if(!nav_result['Sold_to'] || nav_result['Sold_to'] !== sap_payload.SoldTo){
                    logger.info('inside CreateSelfLiftingOrder -> create, Error: SAP response does not contain Sold_to or Sold_to does not match with payload');
                    return {
                        status: false,
                ...
                if(!nav_result['Unloading'] || nav_result['Unloading'] !== sap_payload.Unloading){
                    logger.info('inside CreateSelfLiftingOrder -> create, Error: SAP response does not contain Unloading or Unloading point does not match with payload');
                    return {
                        status: false,
                ...
                if(!updateOrdersResult || updateOrdersResult.rowCount === 0){
                    logger.info('inside CreateSelfLiftingOrder -> create, Error: Failed to update order in database');
                    return {
                        status: false,
                        message: 'Self Lifting order created in SAP but failed to update order in database',
                        data: createOrderResponse.data,
                    };
                }
                logger.info('inside CreateSelfLiftingOrder -> create, Order updated in database successfully');
                ...
                logger.info('inside CreateSelfLiftingOrder -> create, notificationpreferanceResult: ',selectUserNotificationResult?.rows[0]);
                if (!selectUserNotificationResult?.rows?.length) {
                    logger.info('inside CreateSelfLiftingOrder -> create, notification preference does not exists for distributor- '+ distributor_id);
                    continue;
                }
                ...
                if (!adminData || !Object.keys(adminData).length) {
                    logger.info('inside CreateSelfLiftingOrder -> create, Error: Admin data not found for distributor- '+ distributor_id);
                    continue;
                }
                //Sending email
                if (isEmailEnabled) {
            ...
            return {
                status: true,
                message: 'Self-lifting order created successfully',
                data: createOrderResponse.data,
            };
            
        } catch (e) {
            logger.error('inside CreateSelfLiftingOrder -> create, Error: ', e);
            throw e;
        }
    }
}
