...
export class ValidateLiquidationOrder implements ValidateOrderInterface {
    public order_type: OrderTypes  = OrderTypes.LIQUIDATION;
    public orderType: string = 'LIQUIDATION';
    static #instance: ValidateLiquidationOrder;

    private constructor() {
    }

    public static getInstance(): ValidateLiquidationOrder {
        if(!this.#instance){
            this.#instance = new ValidateLiquidationOrder();
        }
    ...
    public validatePayload(order_data: any): { status: boolean; message: string; } {
        ...
        try{
            ...
            logger.error('inside validateLiquidationOrder -> validatePayload, Error: ', e);
            response.status = false;
        ...
        try{
            ...
            if(status === 200 || status === 201){
                ...
                for (let result of validatedOrderResult) {
                    if (isGlobalPDPOn && dbData.enable_pdp && !(result.CMIR_Flag === 'X') && result.PDP_Day) {
                ...
                response.status = false;
                response.message = message || ErrorMessage.VALIDATE_LIQUIDATION;
                response.data = data || null
            }

        }catch(e){
            logger.error('inside validateLiquidationOrder, Error: ', e);
            response.status = false;
            response.message = (<Error>e).message || ErrorMessage.VALIDATE_LIQUIDATION;
            response.data = null;
        }finally{
            return response;
        }
    ...
    public async saveOrder(order_data: any, sap_payload: any, sap_result: any, distributor_id: string, user: any): Promise<{status: boolean, message: string, data: any}> {
        ...
        logger.info('inside validateLiquidationOrder -> saveOrder');
        try{
            ...
            if (insertResult?.rowCount) {
                logger.info(`inside validateLiquidationOrder -> saveOrder, PO: ${order_data.po_number} saved successfully`);
                ...
                logger.info(`inside validateLiquidationOrder -> saveOrder, failed to save PO: ${order_data.po_number}`);
            }
    

        }catch(error){
            logger.error('inside validateLiquidationOrder -> saveOrder, Error: ', error);
